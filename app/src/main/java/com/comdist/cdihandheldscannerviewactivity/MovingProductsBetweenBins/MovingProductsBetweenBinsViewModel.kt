...
class MovingProductsBetweenBinsViewModel: ViewModel() {
    ...
    fun getAllBinsFromBackend(warehouseNumber: Int){
        ...
        val exceptionHandler = CoroutineExceptionHandler { _, exception ->
            ...
            Log.i("Get All Bins - Bin Movement (exceptionHandler) " , "Error -> ${exception.message}")
        }

        // API call to get the products in order
        try {
            viewModelScope.launch(exceptionHandler) {
                val response = ScannerAPI.getMovingItemsBetweenBinsService().getAllBins(warehouseNumber)
